...
class dao_log extends dao_base{
	function getLog( $channel_id, $time = null,  $num = 30  ){
		$sql = "SELECT nick.name as nick, log.log as log, log.created_on as time FROM log JOIN nick ON log.nick_id = nick.id WHERE channel_id = $channel_id";
	...
	function getLogAll( $time ){
		...
		$sql = "SELECT log.channel_id as channel_id, nick.name as nick, log.log as log, log.created_on as time FROM log JOIN nick ON log.nick_id = nick.id WHERE log.created_on > ".$this->qs($time);
		$sql .= " ORDER BY log.created_on DESC";
		return $this->_conn->getArray($sql);
	}

	function searchLog( $word, $channel_id = null ){
		$sql = "SELECT nick.name as nick, channel.name as channel_name, log.log as log, log.created_on as time FROM log JOIN nick ON log.nick_id = nick.id JOIN channel ON log.channel_id = channel.id  WHERE log.log like ".$this->qs("%$word%")." ";
